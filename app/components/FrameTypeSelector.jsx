      ...
      <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
        {Object.values(FRAME_TYPES).map((type) => {
          ...
          return (
            ...
            >
              <span className={`text-xs font-semibold sm:text-sm ${isSelected ? "text-white" : "text-zinc-200"}`}>
                {type.label}
              </span>
              <span className="mt-0.5 text-[10px] leading-snug text-zinc-400 sm:text-xs">
                {type.description}
              </span>
              <span
