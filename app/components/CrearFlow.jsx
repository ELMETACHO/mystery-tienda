...
// Flujo completo de creación del cuadro (subir foto → ajustar zoom/tamaño
// → "Tu cuadro está listo"), extraído de app/crear/page.js para poder
// reutilizarse tal cual en /crear y embebido dentro de /ads (ver
// app/ads/page.js) — misma lógica, sin duplicar nada, solo cambia dónde
// vive visualmente.
//
// `compact`: pensado para vivir embebido a media página (no como página
// completa) — oculta la cabecera grande (texto tipo máquina de escribir +
// DriftWall + "Volver atrás", que no tienen sentido en medio de otra
...
export default function CrearFlow({ compact = false }) {
  ...
  // texto cambia de lugar. No se muestra en absoluto en modo compact.
    ...
    <div
      className={
        compact
          ...
          : "mx-auto flex w-full max-w-6xl flex-1 flex-col px-4 py-4 sm:px-6 sm:py-10"
      }
    >
      {/* En móvil, el título/subtítulos son lo primero que ve el cliente —
          ...
          isMobile es true. Nada de esto aplica en modo compact. */}
      {!compact && isUploadScreen && isMobile && (
      ...
      {/* Cabecera de la primera pantalla (subir foto): texto tipo máquina
          ...
          ambos lado a lado, como siempre. Se omite por completo en modo
          compact (embebido dentro de otra página). */}
      {!compact &&
        (isUploadScreen ? (
      ...
      {readyOrder ? (
        ...
        <div className={`flex flex-col gap-5 sm:gap-6 ${mobileOrderUpload}`}>
          {/* La caja de carga ahora ocupa todo el ancho: el mockup lateral
              ...
              arriba, que ya cumplen ese rol visual). Padding/alturas
              reducidas respecto al original: quedaba demasiado espacio en
              blanco alrededor del botón "Elegir mi fotografía". */}
          <div className="mx-auto grid w-full max-w-xl gap-4 sm:gap-6">
              ...
              >
                {isProcessingFile ? (
                  ...
                  <>
                    ...
                    <label
                      htmlFor="file-upload"
                      className="mt-1 cursor-pointer rounded-full bg-accent px-6 py-2.5 text-sm font-medium text-white transition-colors hover:bg-accent-soft"
                    >
